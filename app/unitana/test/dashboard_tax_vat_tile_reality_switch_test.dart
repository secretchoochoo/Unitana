...
void main() {
  ...
  Finder taxVatTileFinder() {
    return find.byWidgetPredicate((w) {
      return w is UnitanaTile && w.title == 'Tax / VAT';
    });
  }

  testWidgets('Tax VAT tile follows the reality pricing context', (
    tester,
  ) async {
    ...
    await layout.addTool(ToolDefinitions.taxVatHelper);
    ...
    expect(taxVatTileFinder(), findsOneWidget);
    var tile = tester.widget<UnitanaTile>(taxVatTileFinder());

    expect(tile.primary.trim().startsWith('€'), isTrue);
    expect(tile.secondary, contains('23%'));
    expect(tile.secondary, contains('Included'));

    session.setReality(DashboardReality.home);
    await tester.pump(const Duration(milliseconds: 200));

    tile = tester.widget<UnitanaTile>(taxVatTileFinder());
    expect(tile.primary.trim().startsWith(r'$'), isTrue);
    expect(tile.secondary, contains('8%'));
    expect(tile.secondary, contains('Add-on'));
  });
}
