...
void main() {
  ...
  Finder tipHelperTileFinder() {
    return find.byWidgetPredicate((w) {
      return w is UnitanaTile && w.title == 'Tip Helper';
    });
  }

  testWidgets('Tip Helper tile follows the reality pricing context', (
    tester,
  ) async {
    ...
    await layout.addTool(ToolDefinitions.tipHelper);
    ...
    expect(tipHelperTileFinder(), findsOneWidget);
    var tile = tester.widget<UnitanaTile>(tipHelperTileFinder());

    expect(tile.primary.trim().startsWith('€'), isTrue);
    expect(tile.secondary, contains('15%'));

    session.setReality(DashboardReality.home);
    await tester.pump(const Duration(milliseconds: 200));
    tile = tester.widget<UnitanaTile>(tipHelperTileFinder());
