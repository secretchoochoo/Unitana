...
## Addendum: Places Hero refactor rollback (SEV-1)

We attempted a major Dashboard refactor to implement the new Places Hero widget (segmented local/home toggle, dual-reality header, refresh action). The implementation broke the build due to widget API drift and invented or missing types, so we rolled back to the last known-good baseline.

Key takeaways:

- Keep the build green at every micro-step (analyze first, then tests).
- Do not edit widget public APIs while the widget is still being built; add wrappers/adapters or land API changes in a dedicated commit.
- Do not invent domain types (enums/models) without explicitly wiring them into the model layer and imports.
- Prefer parallel implementation: add the new widget next to the old, then switch usage.

See: docs/ai/POSTMORTEM_SEV1_PLACES_HERO_TILE_2025-12-27.md
